...
export const getBorrowDetail = async (request: any) => {
  try {
    ...
    const returnDocuments = await ReturnDetailSchema.findAll({ include: [{ model: ReturnSchema, where: { borrowId: request.borrowId } }], attributes: ['borrowDetailId'], raw: true });
    ...
    const documentIds = await BorrowDetailSchema.findAll({ where: query, raw: true, order: [["id", "ASC"]], attributes: ['documentId'] });
